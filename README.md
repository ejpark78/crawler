...
2.  **Shared Network**: 중앙의 긴 박스는 모든 서비스가 소속된 `crawler_default` 가상 네트워크를 의미합니다.
3.  **Control Center**: `Airflow`의 웹 서버와 스케줄러가 시스템의 중심에서 흐름을 제어합니다.
4.  **Storage & Workers**: 하단에는 실제 데이터를 다루는 `DB(Postgres, Mongo)`와 실제 크롤링을 수행하는 `Worker`가 위치합니다.
5.  **Persistence Layer**: 가장 하단은 컨테이너가 삭제되어도 데이터가 유지되는 호스트의 `Volume` 영역입니다.
